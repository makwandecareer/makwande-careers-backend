...
# app/billing/config.py -> app/billing -> app -> repository root
PROJECT_ROOT = Path(__file__).resolve().parents[2]
ENV_FILE = PROJECT_ROOT / ".env"

# Load the repository .env before reading any billing variables.
...
class BillingSettings:
    ...
    def from_env(cls) -> "BillingSettings":
        ...
        if not database_url:
            raise RuntimeError(
                f"DATABASE_URL is required. Add it to {ENV_FILE} "
                "or define it in the operating-system environment."
            )
