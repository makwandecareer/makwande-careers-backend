...
def improve_cv_summary(
    ...
    return improve_summary(
        ...
        verified_strengths=payload.verified_strengths,
    )


@router.post("/improve-experience", response_model=TextImprovementResponse)
def improve_cv_experience(
    payload: ImproveExperienceRequest,
    _user: dict = Depends(get_current_user),
):
    return improve_experience(
        job_title=payload.job_title,
        duties=payload.duties,
        verified_achievements=payload.verified_achievements,
    )


# ------------------------------------------------------------------
# PROTECTED EXPORTS
# Users MUST have an active trial or premium subscription
# before downloading DOCX or PDF.
# ------------------------------------------------------------------
...
def download_docx(
    ...
    filename = _safe(payload.filename) + ".docx"

    return Response(
        content=export_docx(payload.cv_content),
...
def download_pdf(
    ...
    filename = _safe(payload.filename) + ".pdf"

    return Response(
        content=export_pdf(payload.cv_content),
